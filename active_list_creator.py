...
class ListCreatorPage(tk.Frame):
    ...
    def __init__(self, parent, *args, **kwargs):
        ...
        # A cím
        label = tk.Label(left_container, text="ÉLES TERVEK LÉTREHOZÁSA", bg='white', font=('Helvetica', 20, 'bold'),
                         padx=20,
                         pady=20)
    ...
    def load_plans(self):
        ...
        self.cursor.execute("SELECT plan_name, SUM(hangers_needed) FROM plans WHERE is_draft = 0 GROUP BY plan_name")
            ...
            try:
                ...
                for plan_name, product_id, amount, required_hangers in products_to_save:
                    self.cursor.execute(
                        ...
                        "(?, ?, ?, ?, ?)",
                        (plan_name, product_id, amount, required_hangers, 0)
                    )
                    self.conn.commit()
